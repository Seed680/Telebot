...
注意：`owner_only=False` 只表示 `on_message` 可以监听普通成员消息；`commands` 仍然只能由当前 UserBot 账号 outgoing 指令触发。猜数字、红包、抢答、24 点这类模块应把“普通成员答题/领取口令”放在 `on_message`，把“开局/发放/撤销/结束/帮助”等管理动作放在 `commands`。如果需要自动回复命中关键词后开局，应由 TelePilot 内部自动回复动作触发命令，不允许普通成员直接发送 `{prefix}{command}` 来执行管理动作。

远程模块也要尽量把可配置项暴露出来：主指令、帮助子命令、取消/结束命令、撤销命令、自动删除策略、冷却/超时、帮助/开局/成功/失败/超时模板等都应进入 `config_schema`。帮助模板必须支持 `{prefix}`，不要在文案里硬编码 `,命令`。
...
| `help_command` | string | 帮助子命令或独立帮助指令名 |
| `help_message_template` | string | 帮助文本模板，必须支持 `{prefix}` 和 `{command}` |
...
| `cancel_commands` | array[string] | 取消/强制结束指令别名 |
| `undo_command` | string | 撤销上一步、撤回本轮或回滚最近动作的指令名 |
| `delete_command_message` | boolean | 指令完成后是否删除原指令 |
| `auto_delete_enabled` | boolean | 是否自动删除模块产生的临时消息 |
| `auto_delete_delay_seconds` | integer | 自动删除延迟 |
...
配置页只放稳定配置项：`command`、`help_command`、`cancel_commands`、`undo_command`、`timeout`、`auto_next`、`message_template`、`delete_command_message`、`auto_delete_enabled`、`allowed_chat_ids`。如果确实需要默认奖励，可以命名为 `default_reward`，只作为指令未传奖励时的兜底值。
...
1. 奖励金额优先从触发指令参数读取，例如 `{prefix}game 100`。
