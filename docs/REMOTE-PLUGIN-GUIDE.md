...
涉及 Telegram 侧高风险动作（例如账号 Bot 触发的运维命令）时，还会额外经过 Web Admin 细粒度策略开关与确认链路：
...
远程插件默认使用 `ConfigDialog` 小窗口配置页。这个弹窗的基础表单风格应和 TelePilot 的自定义命令 / LLM 配置页保持同一套体验：
...
```json
{
  "name": "TelePilot Community Plugins",
