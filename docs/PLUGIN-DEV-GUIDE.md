...
> 信任边界：TelePilot 0.x 阶段采用“受信/签名模块同进程运行”的路线。`ctx.client` / `ctx.http` / `ctx.ai` 是权限收口和误用防护，不是零信任沙箱；Registry 不应接收未经 TelePilot/Anoyou 审核的任意 Git 源。若 1.0 之后开放社区市场，插件 subprocess/容器隔离将作为独立 Epic 设计。
...
- `ctx.ai.complete()` 推荐用 `provider_tag` 按用途选择 provider；`provider_hint` / `tag` / `tags` 是兼容别名，新模块不要依赖它们作为主要入口。
